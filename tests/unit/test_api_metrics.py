...
def test_metrics_endpoint_returns_json_diagnostics(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fast_api.app.state.runtime = SimpleNamespace(metrics=metrics)
    monkeypatch.setattr(fast_api.logger, "info", lambda *_args, **_kwargs: None)
...
def test_metrics_endpoint_is_not_registered(monkeypatch: pytest.MonkeyPatch) -> None:
    fast_api.app.state.runtime = SimpleNamespace(metrics=Metrics(time_fn=lambda: 100.0))
    monkeypatch.setattr(fast_api.logger, "info", lambda *_args, **_kwargs: None)
